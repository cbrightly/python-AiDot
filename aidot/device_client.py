...
def _decode_h264_to_jpeg(nal_frames: List["VideoFrame"]) -> Optional[bytes]:
    # Feed a sequence of H.264 NAL-unit VideoFrame objects into a PyAV
    # CodecContext and return JPEG bytes of the first successfully decoded
    # picture.  Frames are passed in arrival order so SPS/PPS precede the
    # keyframe and initialize the decoder correctly.
    # Returns None if no frame decoded; raises ImportError when av is absent.
    try:
        import av  # optional dependency: pip install av
    except ImportError as exc:
        raise ImportError(
            "The 'av' package is required for snapshot decoding. "
            "Install it with:  pip install av"
        ) from exc

    codec = av.CodecContext.create("h264", "r")
    for vf in nal_frames:
        if not vf.is_video or not vf.data:
            continue
        packet = av.Packet(vf.data)
        try:
...
class SnapshotTimer:
    """Periodically captures JPEG snapshots from a live camera stream.

    On each interval the timer opens (or reuses) a live stream, waits for the
    first decodable H.264 keyframe, saves it to *output_dir* as
    ``{device_id}_{epoch_ms}.jpg``, and optionally calls *on_snapshot*.

    Usage::

        timer = await dc.async_start_snapshot_timer(
            output_dir="./snapshots",
            interval=60.0,
            on_snapshot=lambda path, data: print(f"saved {path}"),
        )
        # ... later ...
        await timer.stop()
    """
    ...
    def __init__(
        ...
        output_dir: str,
        interval: float = 60.0,
        on_snapshot: Optional[Callable[[str, bytes], None]] = None,
        frame_timeout: float = 30.0,
    ) -> None:
        self._dc             = device_client
    ...
    async def start(self) -> None:
        """Spawn the background snapshot loop."""
        os.makedirs(self._output_dir, exist_ok=True)
        self._stopped = False
        self._task = asyncio.get_event_loop().create_task(self._snapshot_loop())

    async def stop(self) -> None:
        """Stop the timer and close any open live stream."""
        self._stopped = True
        if self._task and not self._task.done():
            self._task.cancel()
            try:
    ...
    async def _capture_one(self) -> Optional[str]:
        # Open a fresh live stream, buffer frames until we can decode an
        # I-frame, save the JPEG, and return the saved path (or None).
        frame_buf: List[VideoFrame] = []
        keyframe_seen = asyncio.Event()
        ...
        session = await self._dc.async_open_live_stream(
            _on_frame, timeout=self._frame_timeout
        )
        if session is None:
            _LOGGER.warning(
                "SnapshotTimer: could not open live stream for %s",
                self._dc.device_id,
            )
            return None
        ...
        try:
            await asyncio.wait_for(
                keyframe_seen.wait(), timeout=self._frame_timeout
            )
        except asyncio.TimeoutError:
            _LOGGER.warning(
                "SnapshotTimer: no keyframe within %.0fs for %s",
                self._frame_timeout, self._dc.device_id,
            )
            await session.stop()
            return None
        ...
        try:
            jpeg = _decode_h264_to_jpeg(frame_buf)
        except ImportError as exc:
            _LOGGER.error("SnapshotTimer: %s", exc)
            return None

        if not jpeg:
            _LOGGER.warning(
                "SnapshotTimer: H.264 decode produced no image for %s",
                self._dc.device_id,
            )
            return None

        ts_ms = int(time.time() * 1000)
        filename = f"{self._dc.device_id}_{ts_ms}.jpg"
        filepath = os.path.join(self._output_dir, filename)
        with open(filepath, "wb") as fh:
            fh.write(jpeg)

        _LOGGER.info("SnapshotTimer: saved %s (%d bytes)", filepath, len(jpeg))

        if self._on_snapshot:
            try:
                self._on_snapshot(filepath, jpeg)
            except Exception as exc:
                _LOGGER.warning("SnapshotTimer: on_snapshot callback raised: %s", exc)
    ...
    async def _snapshot_loop(self) -> None:
        while not self._stopped:
            try:
                ...
                _LOGGER.error(
                    "SnapshotTimer: unexpected error during capture: %s", exc
                )
            if self._stopped:
                break
            try:
...
class DeviceClient(object):
    ...
    async def async_take_snapshot(
        ...
        # Open live stream, wait for first decodable I-frame, save JPEG to
        # output_path.  Returns True on success.
        #
        # NOTE: depends on async_open_live_stream succeeding.  If the live
        # stream itself cannot deliver frames (e.g. TCP connects but camera
        # sends no data), this will time out and return False.
        ...
        session = await self.async_open_live_stream(
            _on_frame, timeout=frame_timeout
        )
        if session is None:
            _LOGGER.error(
                "async_take_snapshot: could not open live stream for %s",
                self.device_id,
            )
            return False

        try:
            await asyncio.wait_for(keyframe_seen.wait(), timeout=frame_timeout)
        except asyncio.TimeoutError:
            _LOGGER.warning(
                "async_take_snapshot: no keyframe within %.0fs for %s",
                frame_timeout, self.device_id,
            )
            await session.stop()
            return False
        ...
        try:
            ...
            _LOGGER.error("async_take_snapshot: %s", exc)
            return False

        if not jpeg:
            _LOGGER.warning(
                "async_take_snapshot: H.264 decode produced no image for %s",
                self.device_id,
            )
            return False
        ...
        _LOGGER.info(
            "async_take_snapshot: saved %s (%d bytes)", output_path, len(jpeg)
        )
        return True

    async def async_start_snapshot_timer(
        self,
        output_dir: str,
        interval: float = 60.0,
        on_snapshot: Optional[Callable[[str, bytes], None]] = None,
        ...
        # Create, start, and return a SnapshotTimer for this device.
        # Store a reference so async_stop_snapshot_timer() can reach it.
        timer = SnapshotTimer(
            device_client=self,
        ...
        self._snapshot_timer: Optional[SnapshotTimer] = timer
        await timer.start()
        return timer

    async def async_stop_snapshot_timer(self) -> None:
        # Stop any running SnapshotTimer previously started via
        # async_start_snapshot_timer().
        timer: Optional[SnapshotTimer] = getattr(self, "_snapshot_timer", None)
        if timer:
            await timer.stop()
