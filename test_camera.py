...
async def run(args: argparse.Namespace) -> None:
    async with aiohttp.ClientSession() as http_session:
        ...
        for cam in cameras:
            ...
            if args.snapshot and not args.diag_mqtt:
                dev_name = cam.get('name', cam.get('id'))
                print(f"\n[SNAPSHOT] Capturing snapshot for {dev_name} ...")
                import os
                snap_dir = "./snapshots"
                os.makedirs(snap_dir, exist_ok=True)
                ts_ms = int(time.time() * 1000)
                out_path = os.path.join(snap_dir, f"{dc.device_id}_{ts_ms}.jpg")
                ok = await dc.async_take_snapshot(out_path, frame_timeout=args.snapshot_timeout)
                if ok:
            ...
            if args.snapshot_timer and not args.diag_mqtt:
                dev_name = cam.get('name', cam.get('id'))
                interval = args.snapshot_timer
                duration = args.snapshot_timer_duration
                print(f"\n[SNAPSHOT TIMER] {dev_name}: one snapshot every {interval}s for {duration}s ...")

                def _on_snap(filepath: str, data: bytes) -> None:
                    print(f"    snapshot -> {filepath} ({len(data)} bytes)")

                timer = await dc.async_start_snapshot_timer(
                    output_dir="./snapshots",
                    interval=interval,
                    on_snapshot=_on_snap,
                    frame_timeout=args.snapshot_timeout,
                )
                try:
...
def main() -> None:
    ...
    parser.add_argument("--snapshot", action="store_true",
                        help="Capture a single JPEG snapshot to ./snapshots/<device_id>_<ts>.jpg "
                             "(requires: pip install av)")
    parser.add_argument("--snapshot-timeout", type=float, default=30.0,
                        help="Seconds to wait for a decodable keyframe (default: 30)")
    parser.add_argument("--snapshot-timer", type=float, default=0,
                        metavar="INTERVAL",
                        help="Capture a snapshot every INTERVAL seconds (requires: pip install av)")
    parser.add_argument("--snapshot-timer-duration", type=float, default=120.0,
                        metavar="SECONDS",
                        help="How long to run the snapshot timer (default: 120)")
    ...
    if not any([args.p2p, args.list_recordings, args.play, args.live, args.diag_mqtt,
                args.snapshot, args.snapshot_timer]):
        args.p2p             = True
